...
class LAInstall extends Command
{
    ...
    {
        try {
            $this->info('CRM Back-Service installation started...');
            ...
            $this->line("\nMAIL Assistant:");
            if($this->confirm("Want to set your Mail config in the .env file ?", true)) {
                $this->line("MAIL Assistant Initiated....");
                $mail_data = array();

                $mail_data['driver'] = $this->ask('Mail driver', 'smtp');
                $mail_data['host'] = $this->ask('Mail Host', 'mailtrap.io');
                $mail_data['port'] = $this->ask('Mail Port', '2525');

                $mail_data['user'] = $this->ask('Mail user', 'null');
                $mail_data['password'] = $this->ask('Mail password', 'null');
                $mail_data['encyption'] = $this->ask('Mail encryption', 'null');

                LAHelper::setenv("MAIL_DRIVER", $db_data['driver']);
                LAHelper::setenv("MAIL_HOST", $db_data['host']);
                LAHelper::setenv("MAIL_PORT", $db_data['port']);
                LAHelper::setenv("MAIL_USERNAME", $db_data['user']);
                LAHelper::setenv("MAIL_PASSWORD", $db_data['password']);
                LAHelper::setenv("MAIL_ENCRYPTION", $db_data['encryption']);
            }
            ...
            if($this->confirm("This process may change/append to the following of your existing project files:"
                ...
                $this->copyFile($from . "/config/crmbs.php", $to . "/config/crmbs.php");
